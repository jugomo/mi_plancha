...
export class Plancha {
  ...
  private readonly productos = toSignal(this.productosService.listar(), { initialValue: [] as Producto[] });
  ...
  protected readonly items = computed<ItemEnPlancha[]>(() => {
    ...
    return this.enPlancha().map((linea) => {
      ...
      const finEstimado = linea.colocadoEn.toMillis() + (producto?.tiempoCoccionSeg ?? 0) * 1000;
      return {
        id: linea.id,
        nombreProducto: producto?.nombre ?? linea.productoId,
        cantidad: linea.cantidad,
        mesaNumero: linea.mesaNumero,
        capacidadUsada: (producto?.capacidadUnidad ?? 0) * linea.cantidad,
  ...
  protected readonly porTipo = computed<TipoEnCapacidad[]>(() => {
    ...
    for (const item of this.items()) {
      const actual = acumulado.get(item.nombreProducto);
      if (actual) {
        actual.capacidad += item.capacidadUsada;
      } else {
        acumulado.set(item.nombreProducto, {
          productoId: item.id,
