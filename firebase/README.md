...
- **`seed.js`**: script de bootstrap (primer administrador, config del CMS, mesas, ingredientes de referencia) vía Admin SDK, ya que la CLI de Firebase no tiene comandos para escribir documentos Firestore sueltos. Requiere `service-account.json` (no versionado, ver `.gitignore`) generado desde Configuración del proyecto → Cuentas de servicio en la consola. Uso: `npm install && npm run seed`.
